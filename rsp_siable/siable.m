function [circuit, subnormalization, info, global_phase] = siable(Matrix, logging, offset)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    A1 = Matrix ; 
    [W,S,V] = svd(A1) ; 
    D = diag(exp(1j.*acos(diag(S)))) ; 
    
    % Step 3 Construct quantum circuit 

    % M0 = kron([1,1;1,-1]./sqrt(2), eye(pow2(n))) * kron( eye(2), W *
    % kron(eye(pow2(n-2)),Delta') ) * [D,zeros(pow2(n));zeros(pow2(n)), D']
    % * kron( eye(2), kron(eye(pow2(n-2)),Delta) * V' ) *
    % kron([1,1;1,-1]./sqrt(2), eye(pow2(n)))    
    % norm( M0(1:pow2(n),1:pow2(n)) - A1 ) 

    NumQubits = n + 1; 
    circuit = qclab.QCircuit( NumQubits, offset ) ; 
    circuit.push_back( qclab.qgates.Hadamard(0) ) ; 
    % circuitL = qclab.QCircuit( NumQubits, offset ) ; 
    [circuit, global_phase, Delta, nCNOT0] = recursive_decouple_quantum_circuit( circuit, V', n, 1, eye(4), false, logging ) ;
    if logging, info.nCNOT = info.nCNOT + nCNOT0 ; end 
    % M = circuit.matrix; norm( M .* global_phase - kron( eye(2),
    % kron(eye(pow2(n-2)),Delta) * V' ) * kron([1,1;1,-1]./sqrt(2),
    % eye(pow2(n))) )   
    [circuit, nCNOT0] = uniformly_controlled_rotation_z( circuit, D, n+1, 'M', logging ) ;
    if logging, info.nCNOT = info.nCNOT + nCNOT0 ; end 
    % M1 = circuit.matrix; norm(M1 .* global_phase -
    % [D,zeros(pow2(n));zeros(pow2(n)), D'] * kron( eye(2), kron( eye(pow2(n-2)), Delta) * V' ) * kron([1,1;1,-1]./sqrt(2), eye(pow2(n))) ) 
    % circuitR = qclab.QCircuit( NumQubits, offset ) ; 
    [circuit, global_phase, ~, nCNOT0] = recursive_decouple_quantum_circuit( circuit, W, n, global_phase, Delta, true, logging ) ; 
    % M3 = circuit.matrix .* global_phase - kron( eye(2), W * kron(
    % eye(pow2(n-2)), Delta') ) * [D,zeros(pow2(n));zeros(pow2(n)), D'] * kron( eye(2), kron( eye(pow2(n-2)), Delta) * V' ) * kron([1,1;1,-1]./sqrt(2), eye(pow2(n))) 
    % norm( M3 )
    if logging, info.nCNOT = info.nCNOT + nCNOT0 ; end 
    circuit.push_back( qclab.qgates.Hadamard(0) ) ; 
    % M2 = circuit.matrix .* global_phase;  norm( A1 -  M2(1:pow2(n),1:pow2(n)) )
    circuit.push_back( qclab.qgates.RotationZ(0, -2.*angle(global_phase)) ) ; % global phase
